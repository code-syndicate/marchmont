...
const letter: Letter = {
  ...
  action: { label: 'Confirm this address', url: 'https://nashluxuryrealty.house/verify?token=abc' },
}
...
describe('the log provider', () => {
  test('writes the address, the template and the link', async () => {
    ...
    expect(written).toContain('https://nashluxuryrealty.house/verify?token=abc')
  })
})
